...
class AdminWindow(QMainWindow):
    ...
    def send_to_left_Button(self):
        ...
        new_row = [
            QStandardItem(self.model_right.item(row_to_send, col).text())
    ...
    def send_to_right_Button(self):
        ...
        new_row = [
            QStandardItem(self.model_left.item(row_to_send, col).text())
            for col in range(self.model_left.columnCount())
        ]
    ...
    def delete_r(self):
        ...
        try:
            ...
            if result == QMessageBox.Ok:
                self.db.delete_user_right(
                    user_id_r, username_r, user_lastname_r, user_role_r, user_password_r
                )
...
class AddDataWindow(QMainWindow):
    ...
    def append_data(self):
        ...
        role = 'user'
        self.db.add_user(username, last_name, role, password)
    ...
    def append_data_r(self):
        ...
        password = self.lineEdit_3.text()
        role = 'user'
        self.db.add_user_r(username, last_name, role, password)
...
class EditDataWindow(QMainWindow):
    ...
    def edit_data(self):
        try:
            ...
            self.db.update_user(
                self.user_id, new_login, new_username, new_role, new_password
            )
            self.data_edit.emit()
