...
class ProductApp(QMainWindow):
    def __init__(self):
        ...
        conn_str = f"""DRIVER={{ODBC Driver 17 for SQL Server}};
                    ...
                    user=22200322;
                    Database=DB_for_python_lessons;"""
    ...
    def buy_prod(self):
        try:
            ...
            if parsed_data:
                ...
                if count == 0:
                    QMessageBox.warning(
                        self, "Ошибка", "Товар отсутсвует или нет в наличии"
                    )
                    return
    ...
    def load_products(self):
        ...
        for row in self.db.get_info():
            # print(self.db.get_info())
            id, name, price, count, path, imageData = row
            # print(type(imageData))
            # print(str(file_path) + path)
            if count == 0:
            ...
            pixmap = QPixmap()  # Подгрузка изображения
            pixmap.loadFromData(QByteArray(imageData))
            scales_pixmap = pixmap.scaled(100, 100)
            icon = QIcon(scales_pixmap)
            item.setIcon(icon)
